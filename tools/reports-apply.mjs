...
/* הלדג'ר נכתב רק כשבאמת הוחל תיקון.

   קודם הוא נכתב בכל ריצה, וזה יצר PR **בכל שעה**: הקובץ לא קיים
   ב-main, כל ריצה יצרה אותו מחדש, git ראה שינוי, והצינור פתח ענף
   ו-PR שכל תוכנו "שורה אחת ממתינה לעוד מדווחים". שורה שממתינה היא
   בדיוק מה שאמור **לא** לייצר PR.

   מה שנשמר בלי הלדג'ר: שורות שנפסלו על תוכן מסומנות בגיליון עצמו
   כ"נפסל" ולא חוזרות בתור, כך שהלדג'ר הוא מטמון ולא הגבול. הגבול
   הוא עמודת המצב בגיליון. */
if (!DRY && applied.length) writeJSON(LEDGER, ledger);
