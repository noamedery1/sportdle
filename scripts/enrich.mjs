...
for (const club of clubs) {
  ...
  /* --- בריכת התשובות ---
     המפרט המקורי: seasons >= 3 && born != null.
     נוסף pos != null, כי בדיקת הבנייה דורשת ששחקן בבריכה יהיה
     עם עמדה, ובלי זה הבנייה נופלת על הסתירה הזאת.

     ---------- למה 2 ולא 3 ----------
     נמדד 06/09/2026. מה שמוציא שחקן מהבריכה הוא 1,409 פעמים
     "פחות מ-3 עונות" מול 34 פעמים "חסרים נתונים" — כלומר הסף
     הזה, ולא מקור נתונים חסר, הוא מה שקובע את גודל הבריכה.
     המעבר ל-2 מוסיף 318 שחקנים (535 → 853), ולכולם כבר יש
     שנת לידה ועמדה.

     והוא בטוח: הלוח **append-only**, ותוספות נכנסות בסופו
     (ראה `additions` למטה). כלומר בעלי 2 העונות נוחתים אחרי
     כל בעלי 3 העונות, שום חידה שפורסמה לא זזה, והחידות של
     החודשים הקרובים אינן משתנות.

     **לא לרדת ל-1.** שם הבריכה נעשית 1,560 מתוך 1,577 שניתן
     להקליד — כלומר ביטול הסינון. ההרכב שם הוא זרים ששיחקו
     עונה ונעלמו, ורק 29 מתוך 197 בביתר זכו במשהו (מול 15
     מתוך 41 בעלי 3 עונות). ב-versus.js יש רצפה נפרדת של 3
