...
/* ============================================================
   2. המועדונים.
      המאגר מגיע מ-window.SD, ש-boot.js מציב **לפני** הקובץ
      הזה. שם יושבת ההחלטה בין העותק הארוז לעותק מעודכן
      שנמשך מהאתר, כולל האימות. כאן זה כבר נתון מוכן.

      הקריאה סינכרונית ובלי רשת: boot.js מציב את הארוז מיד,
      והרענון נכנס לתוקף בפתיחה הבאה בלבד.
   ============================================================ */
const CLUBS      = window.SD.clubs;
const CLUB_ORDER = window.SD.order;
...
function shareText(){
  ...
  /* /<slug>/ ולא /?club=<slug>: intent-filter באנדרואיד אינו יכול
     להתאים לפי query string, ולכן רק נתיב פותח את האפליקציה בקישור
     ששותפו. הדף הזה קיים ממילא — הוא המשחק עם המועדון נבחר — והוא
     גם מאונדקס, כך שהקישור טוב יותר גם לרשת. */
  return `${head}\n\n${grid}\n\n${key}\n${SITE_URL}/${club.slug}/`;
}
